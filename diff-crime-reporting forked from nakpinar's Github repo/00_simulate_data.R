...
#make a list if estimated parameters
#est_paramas is a list 



###############

generate_fakedata_from_estimated_params<-function(est_paramas,time_steps, reps){
...
mu_bar = est_paramas[1]
theta = est_paramas[2]/est_paramas[3]
omega =est_paramas[3]
sigma_x = est_paramas[4]
sigma_y =  est_paramas[5]
...
# Thin accordingly -- this is the missingness step
...
# Save data
#write.csv(thin_data, file = paste0('Simulated data from estimate parameters by GAN/fake_bogota_victim_data_v', version, '_ts', time_steps, 'run_num_ ',reps, '.csv'), row.names = FALSE)

if(all_crimes==1){
  write.csv(data[data$district!=0,], file = paste0('Simulated data from estimate parameters by GAN/fake_bogota_superset_data_v', version, '_ts', time_steps,'run_num_ ',reps, '.csv'), row.names = FALSE)
}

else{
write.csv(thin_data[thin_data$district!=0,], file = paste0('Simulated data from estimate parameters by GAN/fake_bogota_victim_data_v', version, '_ts', time_steps, 'run_num_ ',reps, '.csv'), row.names = FALSE)
#write.csv(data, file = paste0('Simulated data from estimate parameters by GAN/fake_bogota_superset_data_v', version, '_ts', time_steps,'run_num_ ',reps, '.csv'), row.names = FALSE)
}
...
#generate real data from P_theta_0

generate_realdata_from_estimated_params<-function(real_paramas,time_steps, reps){
 ...
 # assert_that(time_steps/(365/2) == round(time_steps/(365/2)))
  ...
  # Data parameters for high intensity SEPP
  # Data parameters for high intensity SEPP
  mu_bar = real_paramas[1]
  theta = real_paramas[2]/real_paramas[3]
  omega =real_paramas[3]
  sigma_x = real_paramas[4]
  sigma_y =  real_paramas[5]
 ...
 # plan(multiprocess)
  ...
  data_file = paste0("/Users/pramitdas/Desktop/diff-crime-reporting/data/realistic_bogota/data_v", version, '_ts', time_steps, "run_num",  reps,'.csv')
  print(data_file)
  
  #data_file = "/Users/newuser/Desktop/diff-crime-reporting/real tarining data "
  #print(data_file)
  
  # Save data
  if(all_crimes==0){
  
  write.csv(thin_data[thin_data$district!=0,], file = data_file, row.names = FALSE)
  }
  
  else{
    write.csv(data[data$district!=0,], file = data_file, row.names = FALSE)
  }
...
#source("/Users/newuser/Desktop/diff-crime-reporting/dummy.R")
